...
type ObservabilityWorkbenchMode = 'demo';

const OBSERVABILITY_WORKBENCH_MODE: ObservabilityWorkbenchMode = 'demo';
export const WORKBENCH_OBSERVABILITY_MODE_HEADER = 'X-AgentOS-Workbench-Mode';

function markDemoReply(reply: FastifyReply): void {
  reply.header(WORKBENCH_OBSERVABILITY_MODE_HEADER, OBSERVABILITY_WORKBENCH_MODE);
}
    ...
    providerHealth: [
      {
        provider: 'OpenAI',
        status: 'ok',
        latencyMs: 312 + jitter(),
        checkedAt: new Date(now - 30_000).toISOString(),
      },
      {
        provider: 'Anthropic',
        status: 'degraded',
        latencyMs: 890 + jitter(),
        checkedAt: new Date(now - 30_000).toISOString(),
      },
      {
        provider: 'Deepgram',
        status: 'ok',
        latencyMs: 145 + jitter(),
        checkedAt: new Date(now - 30_000).toISOString(),
      },
      {
        provider: 'ElevenLabs',
        status: 'ok',
        latencyMs: 203 + jitter(),
        checkedAt: new Date(now - 30_000).toISOString(),
      },
      {
        provider: 'Qdrant',
...
export default async function observabilityRoutes(fastify: FastifyInstance): Promise<void> {
  ...
  fastify.get(
        ...
        response: {
          200: {
            ...
            properties: {
              ...
              monthSpentUsd: { type: 'number' },
              dailyTokens: { type: 'array', items: { type: 'number' } },
              providerCosts: {
                type: 'array',
    ...
    async (_request, reply) => {
      markDemoReply(reply);
      return {
        mode: OBSERVABILITY_WORKBENCH_MODE,
        ...buildSummary(),
      };
    }
  );
